...
type ApiObjective = { id:string; description:string; type:string; count:number|null; coordinateStatus:"verify"|"unmapped"|"verified"; maps:{id:string;name:string}[];possibleLocations?:unknown[];zones?:unknown[];items?:unknown[];requiredKeys?:unknown[];markerItem?:unknown;usingWeapon?:unknown[];wearing?:unknown[];foundInRaid?:boolean;optional?:boolean };
type ApiTask = { id:string; name:string; minPlayerLevel:number; trader:{id:string;name:string}|null; objectives:ApiObjective[] };
...
type Task = { id:string; title:string; trader:string; objective:string; zone:string; x:number; y:number; world:WorldPoint|null;floor:"B1"|"GROUND"|"UPPER";status:"READY"|"UNMAPPED"|"VERIFY"; color:string; selected:boolean;requirements:{minPlayerLevel:number;objectives:ApiObjective[]} };
...
const initialTasks: Task[] = [
  { id:"demo-1",title:"Checking",trader:"Prapor",objective:"Machinery keyを回収し、Bronze pocket watchを確保",zone:"Customs",x:42,y:37,world:null,floor:"GROUND",status:"READY",color:"#d7ff45",selected:true,requirements:{minPlayerLevel:1,objectives:[]} },
  { id:"demo-2",title:"Delivery from the Past",trader:"Prapor",objective:"Customs officeでsecure caseを回収",zone:"Customs",x:17,y:55,world:null,floor:"GROUND",status:"READY",color:"#ffb547",selected:true,requirements:{minPlayerLevel:1,objectives:[]} },
  { id:"demo-3",title:"The Extortionist",trader:"Skier",objective:"Unknown keyでmessengerの荷物を回収",zone:"Customs",x:63,y:65,world:null,floor:"GROUND",status:"VERIFY",color:"#7dd8ff",selected:true,requirements:{minPlayerLevel:1,objectives:[]} },
];
    ...
    fetch(`/api/tarkov/tasks?lang=ja&map=${encodeURIComponent(map)}&limit=120`,{signal:controller.signal})
      ...
      .then(async (payload) => {
        ...
        let next:Task[] = (payload.data as ApiTask[]).flatMap((task,index) => {
          ...
          return [{id,title:task.name,trader:task.trader?.name ?? "Unknown",objective:objective.description,zone:map,...visual,world,floor:objectiveFloor,status:objective.coordinateStatus==="unmapped"?"UNMAPPED":objective.coordinateStatus==="verified"?"READY":"VERIFY",selected:restoredIds.size?restoredIds.has(id):index<3,requirements:{minPlayerLevel:task.minPlayerLevel,objectives:[objective]}}];
        });
