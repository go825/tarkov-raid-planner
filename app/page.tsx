...
type Task = { id:string; title:string; trader:string; objective:string; zone:string; x:number; y:number; world:WorldPoint|null;floor:"B1"|"GROUND"|"UPPER";status:"READY"|"UNMAPPED"|"VERIFY"; color:string; selected:boolean;requirements:{minPlayerLevel:number;prerequisiteTaskIds:string[];objectives:ApiObjective[]} };
...
type StoredPlan = { map:string; selectedTaskIds:string[]; routeTaskIds:string[];checklistState:Record<string,boolean>;assignments:Record<string,string>;shareId:string;revision:number;members:SquadMember[];ownerUserId:string;currentUserId:string|null };

const initialTasks: Task[] = [
  { id:"demo-1",title:"Checking",trader:"Prapor",objective:"Machinery keyを回収し、Bronze pocket watchを確保",zone:"Customs",x:42,y:37,world:null,floor:"GROUND",status:"READY",color:"#d7ff45",selected:true,requirements:{minPlayerLevel:1,prerequisiteTaskIds:[],objectives:[]} },
  { id:"demo-2",title:"Delivery from the Past",trader:"Prapor",objective:"Customs officeでsecure caseを回収",zone:"Customs",x:17,y:55,world:null,floor:"GROUND",status:"READY",color:"#ffb547",selected:true,requirements:{minPlayerLevel:1,prerequisiteTaskIds:[],objectives:[]} },
  { id:"demo-3",title:"The Extortionist",trader:"Skier",objective:"Unknown keyでmessengerの荷物を回収",zone:"Customs",x:63,y:65,world:null,floor:"GROUND",status:"VERIFY",color:"#7dd8ff",selected:true,requirements:{minPlayerLevel:1,prerequisiteTaskIds:[],objectives:[]} },
];
...
export default function Home() {
  ...
  const newPlan = () => { setTasks((items)=>items.map((task)=>({...task,selected:false}))); setChecklistState({});setAssignments({});setQuery(""); window.history.replaceState(null,"",window.location.pathname); };
