...
export default function Home() {
  ...
  const taskOwnerNames=(taskId:string)=>(taskOwners[taskId]??[]).map((id)=>id==="local-owner"?"LOCAL OWNER":squad.find((member)=>member.userId===id)?.displayName??id).join(", ")||"UNASSIGNED";
    ...
    fetch(`/api/tarkov/tasks?lang=ja&mode=${gameMode}&map=${encodeURIComponent(map)}&limit=500`,{signal:controller.signal})
      ...
      .then(async (payload) => {
        ...
        let next:Task[] = (payload.data as ApiTask[]).flatMap((task,index) => task.objectives.filter((objective)=>objective.mapScope==="any"||objective.maps.some((entry)=>entry.name.toLowerCase()===map.toLowerCase())).map((objective,objectiveIndex)=>{
          ...
          return {id,sourceTaskId:task.id,title:task.name,trader:task.trader?.name ?? "Unknown",objective:objective.description,zone:objective.mapScope==="any"?"Any map":map,...visual,world,floor:objectiveFloor,status:objective.coordinateStatus==="unmapped"?"UNMAPPED" as const:objective.coordinateStatus==="verified"?"READY" as const:"VERIFY" as const,selected:restoredIds.size?restoredIds.has(id):index<3,requirements:{minPlayerLevel:task.minPlayerLevel,prerequisiteTaskIds:task.prerequisiteTaskIds??[],objectives:[objective]}};
        }));
  ...
  useEffect(()=>{const controller=new AbortController();fetch(`/api/tarkov/tasks?lang=ja&mode=${gameMode}&limit=500`,{signal:controller.signal}).then(async(response)=>response.ok?(await response.json() as ApiPayload):null).then((payload)=>{if(payload)setTaskCatalog(payload.data)}).catch(()=>undefined);return()=>controller.abort()},[gameMode]);
  ...
  return <main className="app-shell">
    ...
    <section className={activeNav==="My tasks"?"workspace my-tasks-mode":"workspace"}>
      ...
      <section className="loadout-bar"><span>ROUTE PROFILE</span><label>FACTION<select value={profile.faction} onChange={(event)=>setProfile((current)=>({...current,faction:event.target.value as "pmc"|"scav"}))}><option value="pmc">PMC</option><option value="scav">SCAV</option></select></label><label>LEVEL<input className="level-input" type="number" min="1" max="79" value={profile.playerLevel} onChange={(event)=>setProfile((current)=>({...current,playerLevel:Math.max(1,Math.min(79,Number(event.target.value)||1))}))}/></label><label><input type="checkbox" checked={profile.allowPowered} onChange={(event)=>setProfile((current)=>({...current,allowPowered:event.target.checked}))}/>POWER AVAILABLE</label><label><input type="checkbox" checked={profile.allowConditionalExtracts} onChange={(event)=>setProfile((current)=>({...current,allowConditionalExtracts:event.target.checked}))}/>CONDITIONAL EXTRACTS</label><b>{profile.keyIds.length} KEYS</b><small>鍵マーカーをクリックして所持状態を切替</small></section>
      ...
      {activeNav==="My tasks"&&<section className="my-tasks panel"><div className="my-tasks-head"><div><p className="eyebrow">PLAYER PROGRESSION · MANUAL TRACKING</p><h2>My Tasks</h2></div><div className="mode-switch"><button className={gameMode==="regular"?"on":""} onClick={()=>setGameMode("regular")}>PVP</button><button className={gameMode==="pve"?"on":""} onClick={()=>setGameMode("pve")}>PVE</button></div></div><div className="task-filters">{(["all","available","active","completed","locked"] as const).map((filter)=><button key={filter} className={taskFilter===filter?"on":""} onClick={()=>setTaskFilter(filter)}>{filter.toUpperCase()}</button>)}<span>{filteredCatalog.length} TASKS</span></div><div className="progress-list">{filteredCatalog.map((task)=>{const locked=taskLocked(task);const state=taskStates[task.id]??"available";return <article className={locked?"progress-card locked":"progress-card"} key={task.id}><div className="progress-summary"><span>{locked?"LOCKED":state.toUpperCase()}</span><div><small>{task.trader?.name??"Unknown"} · LEVEL {task.minPlayerLevel}</small><h3>{task.name}</h3><p>{locked?`${task.minPlayerLevel>profile.playerLevel?`Level ${task.minPlayerLevel} required. `:""}${task.prerequisiteTaskIds.filter((id)=>taskStates[id]!=="completed").length} prerequisite(s) incomplete.`:`${task.objectives.length} objectives`}</p></div><select disabled={locked} value={state} onChange={(event)=>setTaskProgress(task,event.target.value as "available"|"active"|"completed")}><option value="available">AVAILABLE</option><option value="active">ACTIVE</option><option value="completed">COMPLETED</option></select></div><div className="objective-progress">{task.objectives.map((objective)=><div key={objective.id}><span>{objective.description}</span><small>{objective.mapScope.toUpperCase()} · {objective.type}</small><select disabled={locked||state!=="active"} value={objectiveStates[objective.id]??"pending"} onChange={(event)=>setObjectiveStates((current)=>({...current,[objective.id]:event.target.value as "pending"|"in_progress"|"completed"|"handover"}))}><option value="pending">PENDING</option><option value="in_progress">IN PROGRESS</option><option value="handover">HANDOVER</option><option value="completed">COMPLETED</option></select></div>)}</div></article>})}</div></section>}
      <div className="planner-grid"><section className="map-panel panel"><div className="panel-head"><div><p className="eyebrow">TACTICAL OVERVIEW · WORLD COORDS</p><h2>{map} route</h2></div><div className="map-view-tools"><div className="pan-tools"><button onClick={()=>setView((value)=>({...value,y:value.y+24}))}>↑</button><button onClick={()=>setView((value)=>({...value,x:value.x+24}))}>←</button><button onClick={()=>setView({zoom:1,x:0,y:0})}>●</button><button onClick={()=>setView((value)=>({...value,x:value.x-24}))}>→</button><button onClick={()=>setView((value)=>({...value,y:value.y-24}))}>↓</button><button onClick={()=>setView((value)=>({...value,zoom:Math.max(1,value.zoom-.25)}))}>−</button><b>{Math.round(view.zoom*100)}%</b><button onClick={()=>setView((value)=>({...value,zoom:Math.min(3,value.zoom+.25)}))}>＋</button></div><div className="floor-tools">{(["ALL","B1","GROUND","UPPER"] as const).map((level)=><button key={level} className={floor===level?"on":""} onClick={()=>setFloor(level)}>{level}</button>)}</div></div></div>
        <div className="map-canvas" aria-label={`${map} tactical map`}><div className="map-viewport" style={{transform:`translate(${view.x}px,${view.y}px) scale(${view.zoom})`}}><div className="terrain terrain-a"/><div className="terrain terrain-b"/><div className="terrain terrain-c"/><div className="road road-main"/><div className="road road-cross"/>
        ...
        </div>{activeTask&&<aside className="objective-detail"><button onClick={()=>setActiveTask(null)}>×</button><p className="eyebrow">{activeTask.trader} · {activeTask.zone}</p><h3>{activeTask.title}</h3><p>{activeTask.objective}</p><dl><div><dt>TYPE</dt><dd>{activeTask.requirements.objectives[0]?.type??"UNKNOWN"}</dd></div><div><dt>FLOOR</dt><dd>{activeTask.floor}</dd></div><div><dt>MAP SCOPE</dt><dd>{activeTask.requirements.objectives[0]?.mapScope??"UNKNOWN"}</dd></div><div><dt>PREREQUISITES</dt><dd>{activeTask.requirements.prerequisiteTaskIds.length||"NONE"}</dd></div><div><dt>TASK OWNERS</dt><dd>{taskOwnerNames(activeTask.id)}</dd></div></dl></aside>}</div></section>
      ...
      <section className="checklist-panel panel"><div className="checklist-head"><div><p className="eyebrow">LOADOUT · OBJECTIVE REQUIREMENTS</p><h2>Pre-raid checklist</h2></div><strong>{completedChecklist}<span> / {checklist.length} READY</span></strong></div><div className="checklist-body">{checklist.length?<div className="checklist-items">{checklist.map((item)=>{const done=Boolean(checklistState[item.id]);return <div className={done?"checklist-row done":"checklist-row"} key={item.id}><button aria-label={`${item.label}を完了`} onClick={()=>setChecklistState((state)=>({...state,[item.id]:!state[item.id]}))}>{done?"✓":""}</button><div><b>{item.label}</b><small>{item.taskTitle}{item.foundInRaid?" · FOUND IN RAID":""}{item.optional?" · OPTIONAL":""}</small></div><span className={`requirement-status ${(done?"ready":item.status).toLowerCase().replace(" ","-")}`}>{done?"READY":item.status}</span><select aria-label={`${item.label}の担当`} value={assignments[item.id]??""} onChange={(event)=>setAssignments((state)=>({...state,[item.id]:event.target.value}))}><option value="">UNASSIGNED</option>{squad.map((member)=><option key={member.userId??member.displayName} value={member.userId??member.displayName}>{member.displayName}</option>)}</select></div>})}</div>:<div className="checklist-empty">タスクを選択すると必要な鍵・装備・レイド内行動を自動生成します。</div>}<aside className="readiness"><p className="eyebrow">SQUAD READINESS</p>{squadReadiness.map((member)=><div key={member.userId??member.displayName}><span>{member.displayName}</span><b>{member.done}/{member.total}</b><i style={{width:`${member.total?member.done/member.total*100:0}%`}}/></div>)}</aside></div></section>
