...
### Phase 24 — タスク・マーカー精度改善（完了）

- [x] マップごとの固定ゲーム座標範囲を導入。
- [x] 動的な座標範囲によるマーカー移動と上下左右反転を解消。
- [x] レイド計画を「進行中」タスクだけに限定。
- [x] 座標未登録・マップ範囲外の仮マーカーを非表示化。
- [x] ESLintエラー0件、本番ビルド成功、自動テスト17件成功。
- [x] GitHubへ`6118b21`を反映し、Sites v22を所有者限定でデプロイ。

### Phase 23 — レイド計画画面の簡素化（完了）

- [x] レイド前チェックリストを画面から削除。
- [x] 分隊ステータスを画面から削除。
- [x] 削除した表示専用処理を整理。
- [x] ESLintエラー0件、本番ビルド成功、自動テスト16件成功。
- [x] GitHubへ`0a9ae7a`を反映し、Sites v21を所有者限定でデプロイ。

### Phase 22 — 操作可能なベクターマップ（完了）

- [x] 静止画像コンポーネントをSVGドキュメントレイヤーへ変更。
- [x] マウス／タッチのドラッグ移動とホイール拡大縮小を追加。
- [x] 矢印キー、＋／－、0キーによるキーボード操作を追加。
- [x] 目標・スポーン・脱出口・危険地点・鍵マーカーのクリック操作を維持。
- [x] ESLintエラー0件、本番ビルド成功、自動テスト16件成功。
- [x] GitHubへ`f91da90`を反映し、Sites v20を所有者限定でデプロイ。

### Phase 21 — コミュニティマップ取得・表示（完了）

- [x] tarkov.dev採用元のSVGマップ11点を取得。
- [x] 対応する10マップの背景としてレイド計画へ統合。
- [x] CC BY-NC-SA 4.0全文、取得元commit、帰属、非商用条件を同梱。
- [x] Creditsへ地図作者・ライセンス・禁止用途を表示。
- [x] ESLintエラー0件、本番ビルド成功、自動テスト16件成功。
- [x] Sites v19を所有者限定でデプロイし、一般公開設定を変更していないことを確認。

### Phase 20 — タスク中心の画面構成（完了）
